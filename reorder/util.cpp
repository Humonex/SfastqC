...
uint32_t read_fastq_block(std::istream *fin, std::string *id_array,
                          ...
                          const uint32_t &num_reads, const bool &fasta_flag) {//读取FASTQ文件的read
  uint32_t num_done = 0;
  std::string comment;
  for (; num_done < num_reads; num_done++) {
    ...
    if (!std::getline(*fin, read_array[num_done]))
      throw std::runtime_error(//当FASTQ文件中存在一些错误的时候报错并且停止程序
...
void write_fastq_block(std::ofstream &fout, std::string *id_array,
                       ...
                       const int &num_thr, const bool &gzip_flag) {//写入fastq文件的read
  if (!gzip_flag) {
    ...
    uint64_t num_reads_per_thread =
        1 + ((num_reads - 1) / num_thr);  // ceiling function//每个thread中的最大read数量
...
void write_dna_in_bits(const std::string &read, std::ofstream &fout) {//将DNA数据替换为Bits
  uint8_t dna2int[128];
  dna2int[(uint8_t)'A'] = 0;
...
void read_dna_from_bits(std::string &read, std::ifstream &fin) {//将bit形式的dna还原为AGCT
  uint16_t readlen;
  uint8_t bitarray[128];
