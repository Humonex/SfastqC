...
struct reorder_global {
  ...
  const int numdict = NUM_DICT_REORDER;//同时开始的reorder个数
  ...
  std::bitset<bitset_size> mask64;  // bitset with 64 bits set to 1 (used in
                                    // bitsettostring for conversion to ullong)//将bitset的内容转化为无符号长整数
...
void bitsettostring(std::bitset<bitset_size> b, char *s, const uint16_t readlen,
                    const reorder_global<bitset_size> &rg) {//将bitset转换为string
  // destroys bitset b
...
template <size_t bitset_size>
void setglobalarrays(reorder_global<bitset_size> &rg) {//设置了一个全局数组
...
{//针对变长的read
  ...
  if (resetcount == true)  // resetcount - unmatched read so start over//因为没有匹配成功，所以重新开始
  {
    ...
    // find max of each position to get ref//在每个位置重复最多次数的base得到ref
...
template <size_t bitset_size>
void readDnaFile(std::bitset<bitset_size> *read, uint16_t *read_lengths,//读取DNAFILE并将其转换为BITSETS
                 const reorder_global<bitset_size> &rg) {
...
bool search_match(const std::bitset<bitset_size> &ref,
                  ...
                  const reorder_global<bitset_size> &rg) {//寻找匹配到的read
  static const unsigned int thresh = THRESH_REORDER;
  const int maxsearch = MAX_SEARCH_REORDER;
  std::bitset<bitset_size> b;
  uint64_t ull;
  int64_t dictidx[2];    // to store the start and end index (end not inclusive)in the dict read_id array index in startpos
           //dictidx是存储开始和结束位置的数组              
  uint64_t startposidx;  
  bool flag = 0;
  for (int l = 0; l < rg.numdict; l++) {
    // check if dictionary index is within bound//检测字典索引是否在范围
    if (!rev) {
...
void reorder(std::bitset<bitset_size> *read, bbhashdict *dict,
             uint16_t *read_lengths, const reorder_global<bitset_size> &rg) {//重排序算法
...
#pragma omp parallel
  {
    ...
    // pick arbitrary read
    //left_search_start（左搜索开始时为true，以避免第一次读取的双重删除）left_search
    //（左搜索时为true）需要，以便我们知道何时选择任意读取
...
#pragma omp critical
    {  // doing initial setup and first read
      ...
      //下面是一些修复程序，以确保在很少读取时（与num_线程相当）不会发生错误。
      //基本上，如果已经读了，这个线程就放弃了
      if (rg.numreads == 0) {
        done = true;
      ...
      if (!stop_searching)
        for (int shift = 0; shift < rg.maxshift; shift++) {
          ...
          flag = search_match<bitset_size>(
              ...
              remainingreads, read, dict, k, false, shift, ref_len, rg);//寻找匹配并且检查是否要更新ref
          if (flag == 1) {
            current = k;
            int ref_len_old = ref_len;
            updaterefcount<bitset_size>(read[current], ref, revref, count,
                                        false, false, shift,
                                        read_lengths[current], ref_len, rg);//更新ref
            if (!left_search) {
